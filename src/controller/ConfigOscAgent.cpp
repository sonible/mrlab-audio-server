...
void ConfigOscAgent::sendConfigmgrList (OscEndpoint* destination)
{
    auto msg = lo::Message();
    ...
    sendOrBroadcast (destination, osc::Address::configmgr_list, msg);
}

void ConfigOscAgent::sendConfigmgrNum (OscEndpoint* destination)
{
    auto msg = osc::Message (int32_t (configController.getConfigurations().size()));

    sendOrBroadcast (destination, osc::Address::configmgr_num, msg);
}

void ConfigOscAgent::addMethods()
{
    // Query the config list.
    addMethod (osc::Address::configmgr_list, {}, [this] (OscEndpoint* source) {
        sendConfigmgrList (source);
    });

    // Query the number of configs.
    addMethod (osc::Address::configmgr_num, {}, [this] (OscEndpoint* source) {
        sendConfigmgrNum (source);
    });

    // Control (so far only repopulate from config dir).
    addMethod (osc::Address::configmgr_control, "s", [this] (OscEndpoint* source, lo_arg** argv, int /*argc*/) {
        std::string_view command = &argv[0]->s;

        if (command != "rescan")
            return sendError (source, osc::Error::configCommandUnknown, command);

        configController.unloadAllConfigurations();
        configController.populateFromConfigDir();
    });

    // Config control, currently only reloading.
    addMethod (std::string (osc::Address::config) += "/*/control", "s", [this] (OscEndpoint* source, std::string_view path, std::string_view /*types*/, lo_arg** argv, int /*argc*/) {
